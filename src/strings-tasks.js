...
function removeFirstOccurrences(str, value) {
  const valueCutPrev = str.indexOf(value);
  let finalStr = str;
  if (valueCutPrev > -1) {
    const valueCutFin = str.indexOf(value) + value.length;
    const word = str.slice(valueCutPrev, valueCutFin);
    finalStr = str.replace(word, '');
  }
  return finalStr;
}
...
function removeLastOccurrences(str, value) {
  const valueCutPrev = str.lastIndexOf(value);
  let finalStr = str;
  if (valueCutPrev > -1) {
    const valueCutFin = str.lastIndexOf(value) + value.length;
    const firstPardFrase = str.slice(0, valueCutPrev);
    const endPardFrase = str.slice(valueCutFin, value.str);
    finalStr = firstPardFrase.concat(endPardFrase);
  }
  return finalStr;
}
